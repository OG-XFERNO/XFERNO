...
/**
 * @title XfernoPair
 * @notice XFERNO DEX liquidity pair with constant product AMM
 * @dev Implements x * y = k invariant with 0.3% swap fee
 */
contract XfernoPair is IXfernoPair, ERC20, ReentrancyGuard {
    // ============ Constants ============
    ...
    constructor() ERC20("XFERNO LP Token", "XFERNO-LP") {
        factory = msg.sender;
    }
     ...
     * @dev Called once by factory at deployment
     */
    ...
    /**
     * @notice Mint LP tokens for provided liquidity
    ...
    function swap(
        ...
        uint256 balance1;
        {
            ...
            // Flash swap callback
            if (data.length > 0) {
                IXfernoCallee(to).xfernoCall(msg.sender, amount0Out, amount1Out, data);
            }
...

        {
            ...
            if (balance0Adjusted * balance1Adjusted < uint256(_reserve0) * _reserve1 * 1000000) revert K();
        }
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo = IXfernoFactory(factory).feeTo();
 ...
 */
interface IXfernoCallee {
    function xfernoCall(address sender, uint256 amount0, uint256 amount1, bytes calldata data) external;
}
